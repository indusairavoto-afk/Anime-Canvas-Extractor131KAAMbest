...
## Durable workaround: direct popup/new-tab instead of server-side proxy (2026-07-03)
Since the CF block is on our *server's* IP (not the end user's), and X-Frame-Options only blocks *framing* (not top-level navigation), the most durable fix for any CF-blocked-but-not-frame-friendly site is: don't fetch it server-side at all — construct the target URL client-side (pure string building, e.g. `https://www.miruro.bz/watch/{anilistId}/{slug}?ep={ep}`) and open it with `window.open()` directly from a user click handler (must be synchronous with the click, or browsers block it as a popup). The end user's real browser/IP makes the request, so CF treats it as a normal visit — no relay/proxy infra needed at all. Added `/api/miruro/direct-url` (no fetch, no CF session, just URL construction) + an `openMiruroDirect()` popup helper in `watch-anilist.tsx` as a permanent fallback button alongside the iframe-proxy approach.
