...
async function handlePipe(request: Request, url: URL): Promise<Response> {
  ...
  if (request.method === "POST") {
    ...
    type PipeBody = { fn?: string; q?: Record<string, unknown>; host?: string; e?: string };
    let body: PipeBody;
    try {
    ...
    if (body.fn) {
      ...
      const payload = { path: body.fn, method: "GET", query: body.q ?? {}, body: null, version: "0.1.0" };
  ...
  if (!e) {
    return Response.json({ error: "fn (or legacy e) is required" }, { status: 400 });
  }
