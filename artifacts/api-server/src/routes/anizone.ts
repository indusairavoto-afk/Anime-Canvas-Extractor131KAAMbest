 ...
 * Searches anizone.to for anime matching the query.
 * Returns { results: [{ slug, title, thumbnail }], query, total }
 */
router.get("/anizone/search", async (req, res) => {
  ...
  try {
    ...
    // We capture title + nearest slug + nearest thumbnail after each getTitle call.
    const getTitleRe = /window\.getTitle\(this\.anmTitles,\s*'([^']+)'\)/g;
    let tm: RegExpExecArray | null;
    while ((tm = getTitleRe.exec(html)) !== null && results.length < limit) {
      const title = tm[1];
      const afterTitle = html.slice(tm.index + tm[0].length, tm.index + tm[0].length + 2000);

      const slugMatch = afterTitle.match(/href="https:\/\/anizone\.to\/anime\/([a-z0-9]+)"/);
      if (!slugMatch) continue;
      const slug = slugMatch[1];
      if (seenSlugs.has(slug)) continue;
      seenSlugs.add(slug);

      const thumbMatch = afterTitle.match(/src="(https:\/\/anizone\.to\/images\/anime\/[^"]+)"/);
 ...
 * Fetches the anizone.to episode page, extracts the HLS stream URL + subtitle
 * tracks, then returns an HTML page with an HLS.js player for embedding in an
 * iframe.
 */
