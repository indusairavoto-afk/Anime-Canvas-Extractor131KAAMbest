...
function PopularCard({ anime, index }: { anime: AniMedia; index: number }) {
  ...
  return (
    ...
    >
      {/* 3-D flip container — pure CSS so transform-style is never broken */}
      <div
      ...
      >
        ...
        >
          ...
          <div className="relative h-24 shrink-0 overflow-hidden">
            <img src={anime.bannerImage || cover} alt="" className="w-full h-full object-cover scale-105" />
            ...
            <div className="absolute bottom-2 left-3 right-3">
              <h3 className="text-white font-serif text-sm leading-snug line-clamp-2 drop-shadow-lg">{title}</h3>
            </div>
          </div>
          ...
          <div className="flex flex-col flex-1 p-3 gap-2 overflow-hidden">
            ...
            <div className="flex flex-wrap gap-1">
              {anime.format && (
                <span className="px-1.5 py-0.5 bg-white/10 text-white/60 text-[8px] font-mono uppercase tracking-wide">{anime.format}</span>
              )}
            ...
            {/* Description */}
            {cleanDesc && (
              <p className="text-[10px] text-white/40 leading-relaxed line-clamp-4 flex-1">{cleanDesc}</p>
            )}
